...
def build_targets(p, targets, model):
    det = model.module.model[-1] if isinstance(model, (nn.parallel.DataParallel, nn.parallel.DistributedDataParallel)) else model.model[-1]
    ...
    at = torch.arange(na, device=targets.device).view(na, 1).repeat(1, nt)  # 确保在同一设备

    g = 0.5  # offset
    style = 'rect4'
    for i in range(det.nl):
        anchors = det.anchors[i].to(targets.device)  # 确保锚框在同一设备
        gain[2:] = torch.tensor(p[i].shape, device=targets.device)[[3, 2, 3, 2]]  # xyxy gain

        # 匹配目标与锚框
        a, t, offsets = [], targets * gain, 0
        if nt:
            r = t[None, :, 4:6] / anchors[:, None]  # wh 比例
            ...
            if style == 'rect2':
                ...
                if j.numel() > 0 and k.numel() > 0 and l.numel() > 0 and m.numel() > 0:
        ...
        # 添加
        indices.append((b, a.to(targets.device), gj, gi))  # 确保 a 在同一设备
        tbox.append(torch.cat((gxy - gij, gwh), 1))  # 边框
        anch.append(anchors[a])  # 确保锚框在同一设备
        tcls.append(c)  # 类别

    return tcls, tbox, indices, anch

def build_targets_wrong(p, targets, model):
...
def output_to_target(output, width, height):
    if isinstance(output, torch.Tensor):
        output = output.detach().cpu().numpy()  # 使用 detach() 避免计算图
